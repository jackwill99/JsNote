...
/*
Todo: Notes for in, for of, forEach, map()
...
//! Advance looping Array of object
...
//! If u don't define a variable in findIndex, it will return 'index and false'
...
//? Number
Number.isInteger(math_x); //! check isInteger
...
//! You want to use event in childNode but if parentNode of your childNode has already event, use event.stopPropagatin()
...
let count; //* to access global variable
let test; //* this also
...
change.addEventListener("click", function () {
  clearInterval(test); //! to change setInterval at first you must clearInterval
  count = val.value;
  inter();
});
...
function Proin(pro1, pro2) {
  (this.name = pro1), (this.university = pro2);
}
...
fetch(url)
  .then((res) => {
    return res.text();
  }) //! res Response Json & Text ; you can look at this in prototype
